...
/**
 * ============================================================================
 * TMF637 Product Inventory API Service - Generated Implementation
 * ============================================================================
 *
 * This file implements the TMF637 Product Inventory Management API
...
module.exports = {
	...
	actions: {
		/**
		 * LIST Action - Retrieve paginated list of products with search and filtering
		 */
		list: {
			scope: ["product.list"],
			rest: {
			...
			async handler(ctx) {
				try {
					...
					const populated = await Promise.all(entities.map((entity) => this.populateProduct(ctx, entity)));
		...
		/**
		 * CREATE Action - Create new product with related entities
		 */
		create: {
			...
			async handler(ctx) {
				...
				try {
					created = await ctx.call("v1.db.product.create", entityData);
					entityId = created.id;
					...
					if (
						...
						const duplicateError = new Error(`Product with these details already exists`);
				...
				try {
					...
					const populated = await this.populateProduct(ctx, created);
		...
		/**
		 * GET Action - Retrieve single product by ID
		 */
		get: {
			scope: ["product.get"],
			rest: {
		...
		/**
		 * REMOVE Action - Delete product and all related entities
		 */
		remove: {
	...
	methods: {
		...
		async buildSearchConditions(ctx, searchTerm, clientId) {
			...
			const nameFields = ["name", "description", "productSerialNumber"];
		 ...
		 */
		async createProductCharacteristic(ctx, entities, parentId, clientId) {
			const ids = [];
			for (const entity of entities) {
				const id = cuid();
				const created = await ctx.call("v1.db.product_characteristic.create", {
					id,
					clientId,
					...entity,
		...
		async createProductPrice(ctx, entities, parentId, clientId) {
			const ids = [];
			for (const entity of entities) {
				const id = cuid();
				const created = await ctx.call("v1.db.product_price.create", {
					id,
					clientId,
					...entity,
					"@type": entity["@type"] || "ProductPrice"
				});
				ids.push(created.id);
			}
			return ids;
		...
		async createProductRelationship(ctx, entities, parentId, clientId) {
			const ids = [];
			for (const entity of entities) {
				const id = cuid();
				const created = await ctx.call("v1.db.product_relationship.create", {
					id,
					clientId,
					...entity,
					"@type": entity["@type"] || "ProductRelationship"
				});
				ids.push(created.id);
			}
			return ids;
		...
		async createProductTerm(ctx, entities, parentId, clientId) {
			const ids = [];
			for (const entity of entities) {
				const id = cuid();
				const created = await ctx.call("v1.db.product_term.create", {
					id,
					clientId,
					...entity,
					"@type": entity["@type"] || "ProductTerm"
				});
				ids.push(created.id);
			}
			return ids;
		...
		async createProductOrderItem(ctx, entities, parentId, clientId) {
			...
			for (const entity of entities) {
				...
				const created = await ctx.call("v1.db.product_order_item.create", {
					...
					clientId,
					...entity,
					"@type": entity["@type"] || "RelatedOrderItem"
				});
				ids.push(created.id);
			}
			return ids;
		...
		async createRealizingResource(ctx, entities, parentId, clientId) {
			const ids = [];
			for (const entity of entities) {
				const id = cuid();
				const created = await ctx.call("v1.db.realizing_resource.create", {
					id,
					clientId,
					...entity,
					"@type": entity["@type"] || "ResourceRef"
				});
				ids.push(created.id);
			}
			return ids;
		...
		async createRealizingService(ctx, entities, parentId, clientId) {
			const ids = [];
			for (const entity of entities) {
				const id = cuid();
				const created = await ctx.call("v1.db.realizing_service.create", {
					id,
					clientId,
					...entity,
					"@type": entity["@type"] || "ServiceRef"
				});
				ids.push(created.id);
			}
			return ids;
		...
		async createPlace(ctx, entities, parentId, clientId) {
			const ids = [];
			for (const entity of entities) {
				const id = cuid();
				const created = await ctx.call("v1.db.place.create", {
					id,
					clientId,
					...entity,
					"@type": entity["@type"] || "RelatedPlaceRefOrValue"
				});
				ids.push(created.id);
			}
			return ids;
		...
		async createAgreementItem(ctx, entities, parentId, clientId) {
			const ids = [];
			for (const entity of entities) {
				const id = cuid();
				const created = await ctx.call("v1.db.agreement_item.create", {
					id,
					clientId,
					...entity,
					"@type": entity["@type"] || "AgreementItemRef"
				});
				ids.push(created.id);
			}
			return ids;
		...
		async applyJsonPatchQuery(ctx, id, clientId) {
			...
			const updated = await ctx.call("v1.db.product.get", { id, clientId });
			const populated = await this.populateProduct(ctx, updated);
			const schemaFiltered = this.mapToSchema(populated);
			const cleanedResponse = this.applyFinalCleanup(schemaFiltered);

			const eventType = statusChanged && updated.status !== oldStatus ? "ProductStateChangeEvent" : "ProductAttributeValueChangeEvent";
		...
		async applyMergeStrategy(ctx, id, clientId) {
			...
			const updated = await ctx.call("v1.db.product.update", {
				id,
				clientId,
				...updates,
				updatedAt: new Date().toISOString()
			...
			const eventType =
				updates.status && updates.status !== existing.status ? "ProductStateChangeEvent" : "ProductAttributeValueChangeEvent";
			...
			await ctx.call("v1.tmf637.event-publisher.publish", {
				...
				event: {
					eventType,
					eventTime: new Date().toISOString(),
					event: { product: cleanedResponse, changedAttributes }
				}
			});
		...
		async applyPatchOperation(ctx, entityId, clientId, existing, operation) {
			...
			if (!relatedEntityTypes.includes(arrayName)) {
				throw new Error(
					`Invalid array field '${arrayName}'. Field does not exist in Product schema. Valid array fields are: ${relatedEntityTypes.join(", ")}`,
					400
				);
			}
		...
		async applySimpleFieldOperation(ctx, entityId, clientId, operation) {
			...
			if (!validSimpleFields.includes(fieldName)) {
				throw new Error(
					`Invalid field '${fieldName}'. Field does not exist in Product schema. Valid fields are: ${validSimpleFields.join(", ")}`,
					400
				);
			}
	...
	started() {
		this.logger.info("TMF637 Product service started");
	}
};
